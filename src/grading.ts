...
export async function preflight(input: GradingInput): Promise<ValidationIssue[]> {
  ...
  for (const file of input.files) {
    ...
    if (file.inspection === 'derived' && (!canSummarize(file.path) || !/^[a-f0-9]{64}$/.test(file.sha256) || !Number.isSafeInteger(file.representation?.text_budget) || (file.representation?.text_budget ?? 0) < 2000 || (file.representation?.text_budget ?? Infinity) > Number(input.profile.settings.model_evidence_bytes ?? 96000) || (file.representation?.byte_length ?? Infinity) > (file.representation?.text_budget ?? 0))) add(file.path, 'representation_invalid', 'Derived evidence has invalid source identity, type or budget.');
  ...
  if (input.profile.settings.artifact_handling === 'bounded-text-v1' && modelBytes > Number(input.profile.settings.model_evidence_bytes ?? 96000)) add('files', 'model_evidence_limit', 'Represented evidence exceeds the bounded AI input allocation.');
...
function comparison(assessment: StageAssessment): string {
  return canonicalJson({ admission: assessment.admission_action, score: assessment.score_status, conduct: assessment.conduct_status, gaps: [...assessment.inspection_gaps].sort(), blockers: assessment.findings.filter(f => f.blocks_merge_or_credit).map(f => [f.rule_id_or_gate, f.path_or_locus]).sort(), outcomes: assessment.outcomes.map(o => ({ ...(o.source_outcome_ids ? {source_outcome_ids:o.source_outcome_ids,acceptance_test:o.acceptance_test,excluded_overlap:o.excluded_overlap} : {}), family: o.family_id, scope: o.scope, tier: o.assessed_tier, category: o.category, aliases: [...o.aliases].sort(), predecessors: [...o.predecessors].sort(), gates: o.gate_evidence.map(g => [g.gate, g.status]).sort(), attribution: o.attribution_status })).sort((a, b) => a.family.localeCompare(b.family)) });
}
...
export function correctionErrors(previous: unknown, corrected: StageAssessment): string[] {
  ...
  for (const outcome of corrected.outcomes) {
    ...
    if (prior) for (const key of ['scope', 'category', 'aliases', 'predecessors', 'attribution_status', 'source_outcome_ids', 'acceptance_test', 'excluded_overlap'] as const) {
