...
export async function buildStageRequest(input: GradingInput, stage: GradingStage, priorReports: StageAssessment[] = [], signal = AbortSignal.timeout(input.profile.max_duration_ms)): Promise<StageRequest> {
  ...
  if (new TextEncoder().encode(payload + SYSTEM_PROMPT + STAGE_PROMPTS[stage] + canonicalJson(assessmentSchema)).byteLength > input.profile.max_input_bytes) throw new Error('input_limit: Complete context exceeds the stage envelope.');
  const digests = await gradingDigests(input);
  return { stage, profile: input.profile, system: SYSTEM_PROMPT, developer: STAGE_PROMPTS[stage], payload, output_schema: assessmentSchema, signal, merit_evidence_digest: digests.merit_evidence_digest };
}
...
export async function runGrading(input: GradingInput, runner?: StageRunner, cache: GradingCache = new MemoryGradingCache()): Promise<GradingReport> {
  ...
  async function stage(name: GradingStage, previous?: StageAssessment[]): Promise<StageAssessment> {
    ...
    const promptDigest = await hashObject({ system: SYSTEM_PROMPT, developer: STAGE_PROMPTS[name], payload, schema: assessmentSchema });
