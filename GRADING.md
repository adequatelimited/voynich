...
Before public scoring, qualified humans establish a reasoned reference set from the published rubric. Run the actual AI pipeline against it and publish exact-tier/gate/scope agreement, false gaming/content flags, missed prohibited cases, prediction-versus-official differences, repeated-run stability and cost/latency by route. Calibration is policy validation, not human assignment of production points. Meet the launch targets in `LAUNCH.md`, resolve material boundary failures and make unresolved limitations visible. Test newly held-out variants during grading-release validation; publish those cases and results after evaluation. Their labels are evaluation material, not secret scoring rules.
