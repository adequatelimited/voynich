...
// Regeneration must not silently deactivate a reviewed release.
try { await readFile('grading/profiles/active.json'); }
catch (error) { if ((error as NodeJS.ErrnoException).code !== 'ENOENT') throw error; await writeFile('grading/profiles/active.json', JSON.stringify(DEFAULT_PROFILE, null, 2) + '\n'); }
